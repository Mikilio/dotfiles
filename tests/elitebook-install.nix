{ withSystem, self, ... }:
{
  flake.checks.x86_64-linux.elitebookInstallTest = withSystem "x86_64-linux" (
    {
      inputs',
      pkgs,
      ...
    ...
    let
      ...
      dependencies = [
        self.nixosConfigurations.elitebook.pkgs.stdenv.drvPath
        (self.nixosConfigurations.elitebook.pkgs.closureInfo { rootPaths = [ ]; }).drvPath
        self.nixosConfigurations.elitebook.config.system.build.toplevel
        self.nixosConfigurations.elitebook.config.system.build.diskoScript
      ] ++ builtins.map (i: i.outPath) (builtins.attrValues self.inputs);

      closureInfo = pkgs.closureInfo { rootPaths = dependencies; };
    in
    pkgs.nixosTest {
      name = "elitebook-installer-test";
      nodes.machine = {
      ...
      testScript = ''
        ...
        machine.succeed("${disko}/bin/disko-install --disk main /dev/vdb --extra-files /tmp/age.key /var/lib/secrets/age.key --flake ${../..}#elitebook")
        # test idempotency
        machine.succeed("${disko}/bin/disko-install --mode mount --disk main /dev/vdb --flake ${../..}#elitebook")
        machine.shutdown()
